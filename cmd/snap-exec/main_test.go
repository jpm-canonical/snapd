...
func (s *snapExecSuite) TestSnapExecAppIntegrationWslPathWorkaround(c *C) {
	dir := c.MkDir()
	dirs.SetRootDir(dir)
	snaptest.MockSnap(c, string(mockYaml), &snap.SideInfo{
		Revision: snap.R("42"),
	...
	// mock /usr/lib/wsl as a bind-mount (different device than /usr/lib)
	restore := snapExec.MockSyscallStat(func(p string, st *syscall.Stat_t) error {
		if strings.HasSuffix(p, "/usr/lib/wsl/") {
			st.Dev = 2
		} else {
			st.Dev = 1
		}
	...
	// ensure that PATH contains /usr/lib/wsl/lib
	...
	c.Assert(foundPath, Not(Equals), "")
	c.Check(foundPath, testutil.Contains, "/usr/lib/wsl/lib")
}

func (s *snapExecSuite) TestSnapExecAppIntegrationWslPathWorkaroundAbsent(c *C) {
	dir := c.MkDir()
	dirs.SetRootDir(dir)
	snaptest.MockSnap(c, string(mockYaml), &snap.SideInfo{
		Revision: snap.R("42"),
	...
	// ensure that PATH is not prefixed with /usr/lib/wsl/lib since we didn't mock the WSL dir
	for _, e := range execEnv {
		if strings.HasPrefix(e, "PATH=") {
			c.Check(e, Not(testutil.Contains), "/usr/lib/wsl/lib")
		}
	}
}
